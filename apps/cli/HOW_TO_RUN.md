...
This creates a global `trelexa` command you can use anywhere:
...
**After linking, you can use `trelexa` from any directory!**

## Option 3: Use pnpm Filter (From Root) 📦

```bash
# From the monorepo root
pnpm --filter trelexa start -- --help
pnpm --filter trelexa start -- posts:list
pnpm --filter trelexa start -- posts:create -c "Hello" -i "twitter-123"
```
...
echo 'export TRELEXA_API_KEY=your_api_key' >> ~/.bashrc
...
### "TRELEXA_API_KEY is not set"

```bash
export TRELEXA_API_KEY=your_key

# Verify it's set
echo $TRELEXA_API_KEY
```
...
- `TRELEXA_API_KEY` - Your Trelexa.ai API key (required for all operations)

### Optional

- `TRELEXA_API_URL` - Custom API endpoint (default: `http://localhost:3000`)
...
```bash
export TRELEXA_API_KEY=your_key
export TRELEXA_API_URL=https://custom-api.com
```

**Permanent (add to shell profile):**
```bash
# For bash
echo 'export TRELEXA_API_KEY=your_key' >> ~/.bashrc
source ~/.bashrc

# For zsh
echo 'export TRELEXA_API_KEY=your_key' >> ~/.zshrc
source ~/.zshrc
```
...
| **Global Link** | `trelexa` (after `pnpm link --global`) | **Recommended** for development |
| **pnpm Filter** | `pnpm --filter trelexa start --` | From monorepo root |
| **npm Global** | `trelexa` (after `npm i -g trelexa`) | After publishing to npm |
| **npx** | `npx trelexa` | One-off usage without installing |
...
# 5. Start using!
trelexa posts:create -c "My first post" -i "twitter-123"
```

Now you can use `trelexa` from anywhere! 🚀
