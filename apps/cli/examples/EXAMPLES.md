# Trelexa CLI - Advanced Examples

This directory contains examples demonstrating the full capabilities of the Trelexa CLI, including posts with comments and multiple media.

## Understanding the Post Structure

The Trelexa.ai API supports a rich post structure:
...
```bash
trelexa posts:create --json examples/post-with-comments.json
```
...
```bash
trelexa posts:create --json examples/multi-platform-post.json
```
...
```bash
trelexa posts:create --json examples/thread-post.json
```
...
for file in posts/*.json; do
  ...
  trelexa posts:create --json "$file"
  sleep 2
done
```
...
2. **Invalid image path** - Ensure images are accessible URLs or uploaded to Trelexa.ai first
