...
1. Log in to your Trelexa.ai account at https://trelexa.ai
...
# Fish
set -x TRELEXA_API_KEY your_api_key_here

# PowerShell
$env:TRELEXA_API_KEY="your_api_key_here"
```
...
```bash
# ~/.bashrc or ~/.zshrc
echo 'export TRELEXA_API_KEY=your_api_key_here' >> ~/.bashrc
source ~/.bashrc
```
...
```bash
# Simple post
trelexa posts:create -c "Hello World!" -i "twitter-123"
...
# With pagination
trelexa posts:list -p 2 -l 20

# Search
trelexa posts:list -s "keyword"
```

### Delete a Post

```bash
trelexa posts:delete abc123xyz
```
...
```bash
trelexa upload ./path/to/image.png
```
...
```bash
# Morning post
trelexa posts:create -c "Good morning!" -s "2024-01-15T09:00:00Z"

# Afternoon post
trelexa posts:create -c "Lunch time update!" -s "2024-01-15T12:00:00Z"

# Evening post
trelexa posts:create -c "Good night!" -s "2024-01-15T20:00:00Z"
```
...
trelexa upload ./my-image.png

# Copy the URL from the response, then create post
trelexa posts:create -c "Check out this image!" --image "url-from-upload"
```
...
trelexa posts:list | jq '.[] | .id'

# Get integration names
trelexa integrations:list | jq '.[] | .provider'
```
...
export TRELEXA_API_URL=https://your-instance.com
...
```
❌ Error: TRELEXA_API_KEY environment variable is required
```
...
trelexa posts:list --help
trelexa posts:delete --help
```
...
- [Trelexa.ai Website](https://trelexa.ai)
- [API Documentation](https://trelexa.ai/api-docs)
