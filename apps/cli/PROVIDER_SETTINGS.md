...
The Trelexa CLI supports platform-specific settings for each integration. Different platforms have different options and requirements.
...
```bash
trelexa posts:create --json post-with-settings.json
```
...
```bash
trelexa posts:create --json reddit-post.json
```
...
```bash
trelexa posts:create --json youtube-video.json
```
